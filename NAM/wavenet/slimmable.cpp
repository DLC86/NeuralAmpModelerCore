...
std::unique_ptr<DSP> SlimmableWavenet::_create_wavenet_for_channels(const std::vector<int>& target_channels)
{
  ...
  model->SetTimeScale(_time_scale);
  return model;
}
...
void SlimmableWavenet::SetTimeScale(const int scale)
{
  _time_scale = std::max(1, scale);
  if (_active_model)
    _active_model->SetTimeScale(_time_scale);
  if (auto pending = _pending_load_acquire())
    pending->model->SetTimeScale(_time_scale);
}
